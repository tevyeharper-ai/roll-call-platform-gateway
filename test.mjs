...
const config = loadConfig({
  PORT:'0', RC_GATEWAY_SERVICE_ID:'roll-call-platform-gateway:p3.3-test', RC_GATEWAY_ENV:'staging',
...
const fetchImpl=async(url,options={})=>{
  if(String(url).startsWith('https://access.test/v1/audit/receipts/')){
  ...
  if(String(url)==='https://events.test/api/platform/omni/work'){
  ...
  if(String(url)==='https://events.test/api/platform/omni/calendar'){
...
test('health remains compatible', async()=>{ const {r,b}=await get('/health'); assert.equal(r.status,200); assert.equal(b.status,'ok'); });
test('metadata preserves consumers and advertises OMNI broker', async()=>{ const {r,b}=await get('/bootstrap/v1/metadata'); assert.equal(r.status,200); assert.deepEqual(b.reference_consumers,consumers); assert.equal(b.omni_read_broker.available,true); });
test('P3.2 ready remains green with P3.3 broker', async()=>{ const {r,b}=await get('/ready'); assert.equal(r.status,200); assert.equal(b.status,'ready'); assert.equal(b.omni_read_ready,true); });
test('OMNI broker readiness is independently green',async()=>{const {r,b}=await get('/v1/omni/readiness');assert.equal(r.status,200);assert.equal(b.organization_id,'roll-call-events');});
for (const consumer of consumers) test(`${consumer} reference remains governed`, async()=>{ const {r,b}=await get(`/v1/${consumer}/reference`); assert.equal(r.status,200); assert.equal(b.consumer,consumer); });
...
test('expired assertion fails', async()=>{ const now=Math.floor(Date.now()/1000); const {r,b}=await post('/v1/identity/assertions/verify',{assertion:sign(claim({iat:now-100,exp:now-1}))}); assert.equal(r.status,401); });
...
test('OMNI Work read verifies receipt then owner domain',async()=>{const {r,b}=await post('/v1/omni/work/read',{access_receipt_id:receiptId},{'x-platform-service-key':omniKey});assert.equal(r.status,200);assert.equal(b.resource,'work');assert.equal(b.access_receipt_id,receiptId);assert.equal(b.data.metrics.upcoming_events,1);});
test('OMNI Calendar read verifies receipt then owner domain',async()=>{const calendarReceipt=receipt({resource:'calendar'});const localFetch=async(url,options={})=>{if(String(url).startsWith('https://access.test/'))return new Response(JSON.stringify({status:'ok',receipt:calendarReceipt}),{status:200,headers:{'content-type':'application/json'}});return fetchImpl(url,options);};const s=createServer(config,{fetchImpl:localFetch});s.listen(0,'127.0.0.1');await once(s,'listening');const r=await fetch(`http://127.0.0.1:${s.address().port}/v1/omni/calendar/read`,{method:'POST',headers:{'content-type':'application/json','x-platform-service-key':omniKey},body:JSON.stringify({access_receipt_id:receiptId})});const b=await r.json();s.close();assert.equal(r.status,200);assert.equal(b.resource,'calendar');assert.deepEqual(b.data.entries,[]);});
